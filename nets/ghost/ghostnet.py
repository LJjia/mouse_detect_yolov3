...
class GhostNet(nn.Module):
    ...
    def __init__(self, cfgs=None, num_classes=1000, width=1.0, dropout=0.2):
...
def classify_test():
    ...
    offical_pth = 'change_official_ghost.pth'
    net = GhostNet()
    model_dict = net.state_dict()
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    # 加载模型到device上
    pretrained_dict = torch.load(offical_pth, map_location=device)
